...
void physics()
{
    ...
    float radius_vec[2], circ_resultant, particle_resultant;
    //if (g.n > 0){
    ...
    // check if particle went off screen...
        ...
        if (particle[i].pos[1] <= box[4].pos[1]-box[4].h) {
            is_box_collision = false;
        }

        radius_vec[0] = (g.xres - particle[i].pos[0]); // x
        radius_vec[1] = (particle[i].pos[1]);  // y
        circ_resultant = sqrt( pow(radius_vec[0], 2) + pow(radius_vec[1],2) );
        particle_resultant = sqrt( pow(particle[i].vel[0], 2) + 
                                    pow(particle[i].vel[1],2) );
        ...
        is_circ_collision = (circ_resultant < g.circ_rad ? true : false);

        if (is_box_collision) {
            particle[i].vel[1] = -particle[i].vel[1] * 0.3f;
            particle[i].vel[0] += 0.01; // so it has a tendency to move right
        } else if (is_circ_collision) {
            // normalize circle resultant
            float bounce_vel[2] = { 
                ( (radius_vec[0] * particle_resultant) / circ_resultant), 
                ( (radius_vec[1] * particle_resultant) / circ_resultant)};
                
            particle[i].vel[1] = bounce_vel[1] * 0.4f;
            particle[i].vel[0] = -bounce_vel[0] * 0.6f;



            //float angle = atan(radius_vec[1]/radius_vec[0]);
            //float resultant_normalized = resultant / resultant;

            // works with incorrect physics
            // particle[i].vel[1] = -particle[i].vel[1] * 0.3f;
            // particle[i].vel[0] = -particle[i].vel[0];

            // cout << "radius_vec[x]: " << radius_vec[0] 
                    //<< ", radius_vec[y]: " << radius_vec[1] << endl;
            // cout << "particle_vel[x]: " << particle[i].vel[0] 
                    //<< ", particle_vel[y]: " << particle[i].vel[1] << endl;
        }
    }
