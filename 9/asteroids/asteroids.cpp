...
// Modified On: 3/21/23
// - textures/colors were added to asteroids
// - a background image texture was also added
...
} img("craters.jpg"), bkg("galaxy.jpg");
...
class Game {
	...
	Game() {
		...
		for (int i=0; i<a->nverts; i++) {
		    ...
		    a->tcoord[i][0] = (sin(angle) / 2.0 + 0.5) ; 
...
void init_opengl(void)
{
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, w, h, 0,
	    GL_RGB, GL_UNSIGNED_BYTE, bkg.data);
...
void buildAsteroidFragment(Asteroid *ta, Asteroid *a)
{
	...
	ta->tcoord[i][0] = (sin(angle) / 2.0 + 0.5) ; 
	ta->tcoord[i][1] = (cos(angle) / 2.0 + 0.5) ; 
	angle += inc;
    }
...
void render()
{
	...
	// used for a variation on coloring the asteroid
    static float gr[8] = {0.4, 0.4, 0.4, 0.4, 0.7, 0.7, 0.7, 0.7};
	static float bl[8] = {0.4, 0.4, 0.4, 0.4, 0.7, 0.7, 0.7, 0.7};
    // float gr_high = 0.7;
    // float gr_low = 0.5;
	// float bl_high = 0.6;
    // float bl_low = 0.4;
    // static float gr_delta = 0.003;
	// static float bl_delta = 0.003;
	static int count = 0;
    ...
    glTexCoord2f(0.0f, 0.0f); glVertex2i(0,  gl.xres);
	...
	// different method to alter asteroid color
	...
	// different method for altering asteroid color
	// gr[0] += gr_delta;
	// bl[0] += bl_delta;
